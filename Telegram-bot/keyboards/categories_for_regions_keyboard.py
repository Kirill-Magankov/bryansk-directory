...
def categories_types_kb(chosen_region):
    ...
    for item in places_types:
        buttons.append([InlineKeyboardButton(text=f"{item['type_name']}", callback_data=f"rt_{chosen_region.split('_')[2]}_{item['type_name']}")])
    buttons.append([InlineKeyboardButton(text="Показать все места района", callback_data=f"rta_{chosen_region.split('_')[2]}")])
