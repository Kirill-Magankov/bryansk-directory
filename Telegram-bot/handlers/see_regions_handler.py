...
async def answer_types_places(callback: CallbackQuery):
    ...
    if response.ok:
        ...
        try:
            ...
            first_place_img = ''
        answer_text =  f"{hide_link('http://localhost:8000/api/v1/places/images/' + first_place_img) if first_place_img != '' else 'Изображение отсутствует'}"\
...
async def answer_types_places(callback: CallbackQuery):
    ...
    if response.ok:
        ...
        try:
            ...
            next_place_img = ''
        answer_text = f"{hide_link('http://localhost:8000/api/v1/places/images/' + next_place_img) if next_place_img != '' else 'Изображение отсутствует'}"\
